...
/// Error type passed by the Chartboost Mediation SDK in its public delegate methods to provide context relative to the failure of a certain operation.
///
/// You may print a `ChartboostMediationError` directly on the console to read the information it contains, or you may read the value of standard
/// `NSError` properties, like `localizedDescription`, `localizedRecoveryOptions`, `localizedRecoverySuggestion`, and `localizedFailureReason`.
...
public class ChartboostMediationError: NSError {
    ...
    /// - Parameter description: Human readable description of the error. This description will be available under the `NSLocalizedDescriptionKey`
    ...
    private init(code: Code, description: String?, error: Error?, errors: [Error]?, data: Data? = nil) {
        ...
        // Limiting the custom description length to prevent bugs or malicious integrations from creating errors with super long strings that may end up getting sent to our backend.
        if let description = description {
            userInfo[NSLocalizedFailureErrorKey] = String(description.prefix(1000)) // prefix result needs to be transformed from Substring to String
        }
    ...
    private override init(domain: String, code: Int, userInfo dict: [String : Any]? = nil) {
