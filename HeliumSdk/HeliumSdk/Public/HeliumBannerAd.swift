...
public protocol HeliumBannerAd {
    ...
    /// ``HeliumBannerAdDelegate/heliumBannerAd(placementName:requestIdentifier:winningBidInfo:didLoadWithError:)``
    /// will be invoked.
    ...
    /// Only one ``HeliumBannerAdDelegate/heliumBannerAd(placementName:requestIdentifier:winningBidInfo:didLoadWithError:)`` call will be
    /// made per `load(with:)` call, even if more ads are loaded as result of the banner auto-refresh process.
