...
public extension PartnerAd {
    ...
    func log(_ event: PartnerAdLogEvent, functionName: StaticString = #function) {
...
public enum PartnerAdLogEvent: ExpressibleByStringInterpolation {
    ...
    /// To log when the partner load result is ignored because it was unexpected or because the ``PartnerAd`` load completion is otherwise unavailable.
    ...
    /// To log when the partner show result is ignored because it was unexpected or because the ``PartnerAd`` show completion is otherwise unavailable.
    ...
    /// - note: You may just pass a `String` in ``PartnerAd/log(_:functionName:)`` to log a custom event instead of using this case directly.
    case custom(String)
