...
final class GCDTaskGroup: DispatchTaskGroup {
    ...
    func onAllFinished(timeout: TimeInterval, execute finished: @escaping () -> Void) {
        ...
        // This is the completion passed in the call to dispatch group notify(), where strongly capturing things causes weird issues where the dispatch group won't stop retaining the completion even after executing it and the group itself being deallocated.
        ...
        // This means we don't need to retain the GCDTaskGroup on the caller side, we can trust that it will remain alive for as long as it is needed.
