...
class AuctionsHTTPRequestFactoryTests: ChartboostMediationTestCase {
    ...
    func testInterstitialBidRequestUsingDefaultMockData() throws {
        ...
        let request = try makeRequest(loadRequest: adLoadRequest, bidderInformation: [:]).body
    ...
    func makeRequest(loadRequest: AdLoadRequest, bidderInformation: BidderInformation) throws -> AuctionsHTTPRequest {
        var request: AuctionsHTTPRequest?
        factory.makeRequest(request: loadRequest, loadRateLimit: 0, bidderInformation: bidderInformation) { httpRequest in
            request = httpRequest
        }
        if let request {
            return request
