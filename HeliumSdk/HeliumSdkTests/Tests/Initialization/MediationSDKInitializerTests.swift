...
class MediationSDKInitializerTests: ChartboostMediationTestCase {
    ...
    func testInitializeSucceedsAfterConfigUpdatesAsLongAsItIsNotTheDefault() {
        ...
        // The info that the initializer should pass to PartnerController merging both configs
        let expectedPartnerConfigurations = [
            "partner1": PartnerConfiguration(credentials: [
                "some key": "some value",
                "asdf": 4,
                "123": ["1", "2", "3"],
                "hey": ["1": "2"]
            ]),
            "partner2": PartnerConfiguration(credentials: [
                "other key": "other value",
                "asdf": "3",
                "2": 23,
                "": 42.42,
                "prebids": [["prebid_key1": "prebid_value1", "prebid_key2": [1, 2, 3]] as [String: Any]]
            ]),
            "partner3": PartnerConfiguration(credentials: [
                "prebids": [
                    ["prebid_key3": "prebid_value3", "prebid_key4": ["a": "b"]] as [String: Any],
                    ["prebid_key5": "prebid_value5"]
                ]
        ...
        // Check that partners are initialized
        XCTAssertMethodCalls(mocks.partnerController, .setUpAdapters, parameters: [expectedPartnerConfigurations, mocks.sdkInitializerConfiguration.partnerAdapterClassNames, Set<PartnerIdentifier>(), XCTMethodIgnoredParameter()])
