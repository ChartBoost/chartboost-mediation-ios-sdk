...
protocol BannerControllerProtocol: AnyObject {
    ...
    /// Partner-specific settings to be sent to adapters in load requests.
    var partnerSettings: [String: Any]? { get set }
    ...
    var request: BannerAdLoadRequest { get }

    /// The `AdLoadResult` of the currently showing ad, or `nil` if an ad is not being shown.
    var showingBannerAdLoadResult: InternalAdLoadResult? { get }
...
final class BannerController: BannerControllerProtocol,
    ...
    private(set) var showingBannerAdLoadResult: InternalAdLoadResult?
    ...
    private var loadCompletion: ((BannerAdLoadResult) -> Void)?
    ...
    init(
        ...
        adController.delegate = self  // to receive ad life-cycle events
    ...
    private func loadAdAndShowIfNeeded(with request: InternalAdLoadRequest) {
        ...
        adController.loadAd(request: request, viewController: viewController) { [weak self] result in
            self?.taskDispatcher.async(on: .background) {
                ...
                if case .success(let ad) = result.result, let view = ad.bannerView {
                ...
                // Load failure: schedule a load retry unless autorefresh is not enabled, or clearAd() was called.
                } else if self.isAutoRefreshEnabled && !self.needsToStopLoadRetryCycle {
    ...
    private func callLoadCompletionIfNeeded(
        ...
        case nil:
            result = BannerAdLoadResult(
                ...
                // contained in `showingBannerAdLoadResult`.
                size: try? showingBannerAdLoadResult?.result.get().bannerSize,
                winningBidInfo: try? showingBannerAdLoadResult?.result.get().bidInfo
            )
    ...
    private func showAd(_ ad: LoadedAd, bannerView: UIView, result: InternalAdLoadResult) {
        taskDispatcher.async(on: .main) { [self] in
            ...
            adController.clearShowingAd { [weak self] _ in
                ...
                self?.taskDispatcher.async(on: .main) {
                    self?.showingBannerAdLoadResult = result
            ...
            visibilityTracker.startTracking(bannerView) { [weak self] in
                self?.taskDispatcher.async(on: .background) {
                    ...
                    // Log metrics
                    _ = self.metrics.logShow(for: ad, start: start, error: nil)
    ...
    private func removeShowingBanner() {
        guard case .success(let ad) = showingBannerAdLoadResult?.result,
              let bannerView = ad.bannerView else
        {
            return
        }
        ...
        self.showingBannerAdLoadResult = nil
    }
    ...
    private func makeLoadRequest() -> InternalAdLoadRequest {
        InternalAdLoadRequest(
            ...
            partnerSettings: self.partnerSettings ?? [:]
        )
    }

    private func updatePausedStates() {
        isLoadRetryTaskPaused = isPaused
