...
protocol BannerSwapControllerProtocol: AnyObject {
...
class BannerSwapController: BannerSwapControllerProtocol {
    ...
    private enum State {
        ...
        case swapping(
            ...
            completion: (BannerAdLoadResult) -> Void
        )
    ...
    var partnerSettings: [String: Any]? {
        // Since the controller can change over time, this controller must be the source of truth
        // of the partnerSettings. We will update them on the controller(s) when they change.
        didSet {
            switch state {
            case .active(let activeController):
                activeController.partnerSettings = partnerSettings
            case .swapping(let pendingController, let activeController, _):
                activeController?.partnerSettings = partnerSettings
                pendingController.partnerSettings = partnerSettings
            case .cleared:
                break
            }
        }
    ...
    var showingBannerAdLoadResult: InternalAdLoadResult? {
        activeController?.showingBannerAdLoadResult
    }
    ...
    func loadAd(
        request: BannerAdLoadRequest,
        viewController: UIViewController,
        completion newCompletion: @escaping (BannerAdLoadResult) -> Void
    ) {
        switch state {
