...
final class NotificationCenterInitResultsEventPublisher: InitResultsEventPublisher {
    ...
    /// Fires the `chartboostMediationDidReceivePartnerAdapterInitResults` notification on the main thread.
    func postInitResultsEvent(_ event: InitResultsEvent) {
        do {
            ...
            taskDispatcher.async(on: .main) {
                NotificationCenter.default.post(name: .chartboostMediationDidReceivePartnerAdapterInitResults, object: json)
            }
        } catch {
