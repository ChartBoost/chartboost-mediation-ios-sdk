...
@objc(CBMFullscreenAdQueueDelegate)
public protocol FullscreenAdQueueDelegate: AnyObject {
    ...
    /// - parameter result: Ad load result with load id, metrics, and error information (if applicable).
    /// - parameter numberOfAdsReady: Current count of loaded ads in the queue.
    @objc optional
    func fullscreenAdQueue(
        _ adQueue: FullscreenAdQueue,
        didFinishLoadingWithResult result: AdLoadResult,
