...
extension PartnerAdapter {
    ...
    public func log(_ event: PartnerLogEvent, functionName: StaticString = #function) {
...
public enum PartnerLogEvent: ExpressibleByStringInterpolation {
    ...
    case fetchBidderInfoStarted(PartnerAdPreBidRequest)
    /// To log when the partner finishes fetching bidder info successfully.
    case fetchBidderInfoSucceeded(PartnerAdPreBidRequest)
    /// To log when the partner finishes fetching bidder info with a failure.
    case fetchBidderInfoFailed(PartnerAdPreBidRequest, error: Error)
    /// To log when the adapter cannot handle a fetch bidder info operation because it's not supported by the partner.
    ...
    /// To log when a partner delegate method call is ignored by the adapter.
    case delegateCallIgnored

    /// To log when the adapter cannot load another ad with the same placement as one already loaded.
    case skippedLoadForAlreadyLoadingPlacement(PartnerAdLoadRequest)

    /// A custom log message for events that do not correspond to any of the predefined event `cases`.
    /// - note: You may just pass a `String` in ``PartnerAdapter/log(_:functionName:)`` to log a custom event instead of using this
    /// case directly.
    case custom(String)
