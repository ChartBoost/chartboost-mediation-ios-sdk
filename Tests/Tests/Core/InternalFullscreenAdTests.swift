...
final class FullscreenAdTests: ChartboostMediationTestCase {
    ...
    let request = FullscreenAdLoadRequest(placement: "some placement", keywords: ["some": "keywords"])
    ...
    func testShowSucceedsWhenControllerSucceeds() {
        ...
        var completion: (InternalAdShowResult) -> Void = { _ in }
        ...
        completion(InternalAdShowResult(error: nil, metrics: nil))
    ...
    func testShowFailsWhenControllerFails() {
        ...
        completion(InternalAdShowResult(error: expectedError, metrics: expectedRawMetrics))
    ...
    /// Validates that forceInternalExpiration() calls are forwarded to the controller.
    func testForceInternalExpiration() {
        ad.forceInternalExpiration()

        XCTAssertMethodCalls(mocks.adController, .forceInternalExpiration)
    }
