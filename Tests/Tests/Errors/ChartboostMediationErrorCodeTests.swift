...
extension ChartboostMediationError.Code {
    ...
    static let defaultInitMessage = "Initialization has failed."
    static let defaultPrebidMessage = "Partner token fetch has failed."
    static let defaultLoadMessage = "Ad load has failed."
    static let defaultShowMessage = "Ad show has failed."
    static let defaultInvalidateMessage = "Ad invalidation has failed."
    ...
    static let rawExpectedData: [Self: ExpectedData] = [
        // 100
        .initializationFailureUnknown: ExpectedData(["CM_100", "CM_INITIALIZATION_FAILURE_UNKNOWN", defaultInitMessage, "There was an error that was not accounted for.", "Try again. If the problem persists, contact Chartboost Mediation Support and provide your console logs."]),
        ...
        .initializationFailureNetworkingError: ExpectedData(["CM_112", "CM_INITIALIZATION_FAILURE_NETWORKING_ERROR", defaultInitMessage, "Init request failed due to a networking error.", "Typically this error should resolve by itself. If the error persists, contact Chartboost Mediation support and share a copy of your network traffic logs."]),
        .initializationFailureOSVersionNotSupported: ExpectedData(["CM_113", "CM_INITIALIZATION_FAILURE_OS_VERSION_NOT_SUPPORTED", defaultInitMessage, "The partner does not support this OS version.", "This is an expected error and can be ignored. Devices running newer OS versions should work fine."]),
        .initializationFailureServerError: ExpectedData(["CM_114", "CM_INITIALIZATION_FAILURE_SERVER_ERROR", defaultInitMessage, "The initialization request failed due to a server error.", "If this problem persists, reach out to Chartboost Mediation Support and/or the mediation partner team for further assistance. If possible, always share a copy of your network traffic logs."]),
        .initializationFailureInternalError: ExpectedData(["CM_115", "CM_INITIALIZATION_FAILURE_INTERNAL_ERROR", defaultInitMessage, "An error occurred within the Chartboost Mediation initialization sequence.", "This is an internal error. Contact Chartboost Mediation Support and provide a copy of your console logs."]),
        .initializationFailureInitializationInProgress: ExpectedData(["CM_116", "CM_INITIALIZATION_FAILURE_INITIALIZATION_IN_PROGRESS", defaultInitMessage, "Multiple initialization requests were made.", "Chartboost Mediation is already initializing. Please wait."]),
        .initializationFailureInitializationDisabled: ExpectedData(["CM_117", "CM_INITIALIZATION_FAILURE_INITIALIZATION_DISABLED", defaultInitMessage, "Initialization has been disabled by the mediation server.", "Update to a newer Chartboost Mediation SDK version or contact Chartboost Mediation Support for assistance."]),
        ...
        .loadFailureNoBannerView: ExpectedData(["CM_329", "CM_LOAD_FAILURE_NO_BANNER_VIEW", defaultLoadMessage, "The partner returns a banner ad with no view to show.", "This is typically caused by a partner adapter bug. Contact the mediation partner and provide a copy of your console logs."]),
        ...
        .loadFailureWaterfallExhaustedNoFill: ExpectedData(["CM_335", "CM_LOAD_FAILURE_WATERFALL_EXHAUSTED_NO_FILL", defaultLoadMessage, "All waterfall entries have resulted in an error or no fill.", "Try again. If the problem persists, verify Partner settings in the Chartboost Mediation dashboard."]),
        ...
        .loadFailureSDKDisabled: ExpectedData(["CM_337", "CM_LOAD_FAILURE_SDK_DISABLED", defaultLoadMessage, "The load request failed due to the Chartboost Mediation SDK being disabled by the mediation server.", "Update to a newer Chartboost Mediation SDK version or contact Chartboost Mediation Support for assistance."]),
