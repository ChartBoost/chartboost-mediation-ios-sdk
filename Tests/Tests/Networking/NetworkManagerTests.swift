...
final class NetworkManagerTests: ChartboostMediationTestCase {
    ...
    func testRequestHeaders() throws {
        ...
        let requests = [
            ...
            HTTPRequestWithRawDataResponseMock(urlString: Self.mockURLString, shouldIncludeMediationHeaders: false),
        ]
        ...
        let headersWithoutMediationHeaders = [
            "Accept": "application/json; charset=utf-8",
            "Content-Type": "application/json; charset=utf-8",
        ]
        ...
        for (index, httpRequest) in requests.enumerated() {
            ...
            try URLProtocolMock.registerHTTPRequest(httpRequest) { request in
                ...
                case 0:
                    XCTAssertEqual(try XCTUnwrap(request.allHTTPHeaderFields), headersWithMediationHeaders)
                case 1:
                    XCTAssertEqual(try XCTUnwrap(request.allHTTPHeaderFields), headersWithoutMediationHeaders)
                default:
    ...
    func testRequestHeadersWhenIDFVIsNil() throws {
        ...
        let httpRequest = HTTPRequestWithRawDataResponseMock(urlString: Self.mockURLString, shouldIncludeMediationHeaders: true)
